...
<!--&lt;!&ndash;            <td class="ps-3"><code>{{ appt.code }}</code></td>&ndash;&gt;-->
...
<!--&lt;!&ndash;            <td>{{ appt.createdBy?.name || '—' }}</td>&ndash;&gt;-->
<!--&lt;!&ndash;            <td>{{ appt.approvedBy?.name || '—' }}</td>&ndash;&gt;-->
<!--            <td>{{ formatDate(appt.appointment_start_date || '—') }}</td>-->
<!--            <td>{{ formatTime(appt.start_time || '—') }}</td>-->
<!--            <td>{{ formatTime(appt.end_time || '—') }}</td>-->
<!--            <td><span :class="['ams-badge', appt.status]">-->
<!--&lt;!&ndash;            </span></td>&ndash;&gt;-->
<!--&lt;!&ndash;            <td><span :class="['badge',&ndash;&gt;-->
<!--&lt;!&ndash;                  appt.status === 'COMPLETED' ? 'bg-success' :&ndash;&gt;-->
<!--&lt;!&ndash;                  appt.status === 'APPROVED' ? 'bg-success' :&ndash;&gt;-->
<!--&lt;!&ndash;                  appt.status === 'IN_PROGRESS' ? 'bg-info' :&ndash;&gt;-->
<!--&lt;!&ndash;                  appt.status === 'PENDING' ? 'bg-warning text-dark' :&ndash;&gt;-->
<!--&lt;!&ndash;                  appt.status === 'REJECTED' ? 'bg-danger' :&ndash;&gt;-->
<!--&lt;!&ndash;                  appt.status === 'CANCELLED' ? 'bg-dark' :&ndash;&gt;-->
<!--&lt;!&ndash;                  appt.status === 'RESCHEDULED' ? 'bg-secondary' :&ndash;&gt;-->
<!--&lt;!&ndash;                  'bg-light text-dark'&ndash;&gt;-->
<!--&lt;!&ndash;                  ]">&ndash;&gt;-->
...
<!--            &lt;!&ndash; Appointment summary &ndash;&gt;-->
...
<!--              &lt;!&ndash; No staff available &ndash;&gt;-->
...
<!--              </div>-->
<!--              &lt;!&ndash;      options for other slots        &ndash;&gt;-->
...
<!--            &lt;!&ndash; Reschedule sub-form (shown when no staff OR user clicked "Send Reschedule") &ndash;&gt;-->
...
<!--            &lt;!&ndash; When staff is available and one is selected: Approve &ndash;&gt;-->
...
<!--            &lt;!&ndash; When no staff: primary action is reschedule &ndash;&gt;-->
...
<!--  try {-->
<!--    console.log(-->
<!--        `/appointments/${appt.code}/availability`-->
<!--    )-->
...
<!--    const alternatives =-->
<!--        res.data.data?.alternatives || {}-->

<!--    alternativeTimeSameLocation.value =-->
<!--        alternatives.different_time_same_location || []-->

<!--    alternativeLocationSameTime.value =-->
<!--        Object.entries(-->
<!--            alternatives.different_location_same_time || {}-->
<!--        ).map(([location_code, staff]) => ({-->
<!--          location_code,-->
<!--          staff-->
<!--        }))-->

<!--    console.log(-->
<!--        'alternativeLocationSameTime',-->
<!--        alternativeLocationSameTime.value-->
<!--    )-->

<!--    console.log(-->
<!--        'serviceLocationsAlternatives',-->
<!--        serviceLocationsAlternatives.value-->
<!--    )-->

<!--    console.log(-->
<!--        'alternativeTimeSameLocation',-->
<!--        alternativeTimeSameLocation.value-->
<!--    )-->

<!--    serviceLocationsAlternatives.value =-->
<!--        alternatives.selected_service_other_locations || []-->

<!--    pickRecommendedAlternative()-->

<!--    selectedChargeCodes.value = []-->

<!--    slotAlreadyBooked.value =-->
<!--        res.data.data?.slot_already_booked || false-->

<!--    conflictingAppointments.value =-->
<!--        res.data.data?.conflicting_appointments || []-->
...
<!--  approvalError.value = ''-->
<!--  try {-->
<!--    await api.post(`/appointments/${selected.value.code}/approve`, { staff_code: approvalSelectedStaff.value, selected_charge_codes: selectedChargeCodes.value })-->
<!--    showApproval.value = false-->
<!--    await fetchAppointments()-->
<!--  } catch (err) {-->
              ...
              <div v-if="autoCharges.length || optionalCharges.length" class="card border-0 bg-light mb-3">
                <div class="card-body">
                  ...
                  <div v-if="optionalCharges.length">
                    <div class="fw-semibold mb-2">Optional Charges</div>
                    <div v-for="charge in optionalCharges" :key="charge.charge_code" class="form-check mb-2">
                      <input :id="`charge-${charge.charge_code}`" v-model="selectedChargeCodes" :value="charge.charge_code" type="checkbox" class="form-check-input" />
                      <label class="form-check-label" :for="`charge-${charge.charge_code}`">
                        {{ charge.name }}
                        <span class="text-muted">( {{ charge.charge_uom === 'percentage' ? `${charge.charge_value}%` : charge.charge_value }} )</span>
                      </label>
